...
move agent::play(board &Board) {
    ...
    if(isAI) {                                                          // ai
        ...
        for(i = 0; i < moves.size(); i++) {
            game->printMoves(i, moves[i]);                              //print out all legal moves
        }
        if(moves.empty()) {
            Move.agentID = game->getCurrentPlayer();
            Move.x_route[0] = Move.y_route[0] = 0xFFU;
            moved = true;
            game->gameOver();                                           //no more moves, GG
        } else if(moves.size() == 1) {
            Move = moves[0];                                            //only move
            maxDepthReached = 0;
        } else {
            Move = alpha_beta_search(Board);                            //so many possibilities...
                                                                        //here comes alpha beta
        }
        if(!moves.empty()){
            std::cout << "AI has chosen move: ";
            game->printMoves(-1, Move);                                 //our AI has made an decision
        }
    } else {                                                            //human
        moved = false;
        moves = game->getMoves(Board, game->getCurrentPlayer());        //get all legal moves
        ...
        while(!moved) {
            ...
            if(inStr.c_str()[0] == 'S' || inStr.c_str()[0] == 's') {
                game->saveGame();
            } else if(inStr.c_str()[0] == 'f' || inStr.c_str()[0] == 'F') {
                Move.agentID = game->getCurrentPlayer();
                Move.x_route[0] = Move.y_route[0] = 0xFFU;
                moved = true;
...
move agent::alpha_beta_search(board &Board) {
    ...
    do {                                                                            //minimax alpha beta search
        // clear root                                                               //as demonstrated in the book
        depth = 0;
        ...
        while(tree_nodes[0].move_index < tree_nodes[0].numMoves && remainingTime > 0.1) {
            if( tree_nodes[depth].beta <= tree_nodes[depth].alpha || 
                ...
                if(depth + 1 < max_depth) {
                    ...
                    score = score_board(tree_nodes[depth + 1].node_board);
                    if(tree_nodes[depth].is_max) {
    ...
    //setMoveTime(double(timeDiff.count()) * duration::period::num / duration::period::den);
    return moveToMake;
}

//heuristic function
int agent::score_board(board &Board) {
    int i, j;
    int score[2] = {0};
    int pieceCount[2] = {0};
        ...
        for(j = 0; j < 12; j++) {
            ...
            if(temp.xCoordinate <= 7 && temp.yCoordinate <= 7) {
                ...
                if (temp.king) {
                    ...
                    score[i] += 1000 + (50 * abs(7 * i - temp.yCoordinate));                     //try to turn king
                ...
                score[i] -= 30 * (abs(4 - temp.xCoordinate) + abs(4 - temp.yCoordinate));
    ...
    return score[currentPlayer] - score[opponent];
}
