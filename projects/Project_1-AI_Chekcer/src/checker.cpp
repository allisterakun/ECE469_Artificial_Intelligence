...
void checker::printBoard(){
    ...
    for(i = 7; i >= 0; i--) {
        ...
        std::cout << termcolor::reset << std::endl << std::setw(13);
        std::cout << " " << rang::bgB::black << termcolor::bold << (i + 1) 
        ...
        for(j = 0; j < 8; j++) {
            ...
            if(Board.gameBoard[i][j] & 1) {
                std::cout << termcolor::white << pawnSymbol[Board.gameBoard[i][j]];
            } else {
                std::cout << termcolor::red << pawnSymbol[Board.gameBoard[i][j]];
            }
            std::cout << "   ";
        }

        std::cout << termcolor::reset << std::endl << std::setw(22);
        for(j = 0; j < 8; ++j) {
            if((j + i) & 1) {
                std::cout << termcolor::on_white << "       ";
            } else {
